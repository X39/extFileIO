...
0 call {
    ...
    switch _returnCode do {
        case -1: { _result = (parseSimpleArray ("[" + _resultData + "]")) select 0; throw (_result select 0); };
...
while { !isNil "_longResult" } do
{
    ...
    if (_errorCode != 0) then { throw _errorCode; };
    switch _returnCode do {
        case -1: { _result = (parseSimpleArray ("[" +_result + _resultData + "]")) select 0; throw (_result select 0); };
